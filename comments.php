...
<?php if ( have_comments() ) : ?>
	<div class="comment-title gdl-link-title gdl-title"><?php comments_number('No Comment', 'One Comment', '% Comments' );?></div>
...
<!-- Comment Form -->
<?php 
	global $gdl_current_page;
	global $gdl_admin_translator;
	
	if( $gdl_admin_translator == 'enable' ){
		$translate_leave_reply = get_option(THEME_SHORT_NAME.'_translator_leave_reply', 'Leave a Reply');
	}else{
		$translate_leave_reply = __('Leave a Reply','gdl_front_end');
	}
	...
	$comment_form = array( 
		'fields' => apply_filters( 'comment_form_default_fields', array(
			...
			'cookies' => '<p class="comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
	            '<label for="wp-comment-cookies-consent">' . esc_html__('Save my name, email, and website in this browser for the next time I comment.', 'gdl_front_end') . '</label></p>',	
			 ) ),
		'comment_field' => '<div class="comment-form-comment">' .
					'<textarea id="comment" name="comment" aria-required="true"></textarea>' .
					'</div><!-- #form-section-comment .form-section -->',
		'comment_notes_before' => '',
		'comment_notes_after' => '<a id="gdl-refresh-contact" class="hidden-item" data-gdlajax="' . $gdl_current_page . '" data-gdlanchor="gdl-comment-zone" ></a>',
		'title_reply' => __( $translate_leave_reply ),
