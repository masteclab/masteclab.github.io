	...
	<?php 
		...
		if($used_font != 'default -'){
			if($all_font[$used_font]['type'] == 'Cufon'){
				echo "Cufon.replace(jQuery(this).find('h1, h2, h3, h4, h5, h6, .gdl-title').not('.nivo-caption .gdl-title'), {fontFamily: '" . $used_font . "' , hover: true});";
			}
		}
		...
		if($used_font != 'default -'){
			if($all_font[$used_font]['type'] == 'Cufon'){
				echo "Cufon.replace(jQuery(this).find('.stunning-text-title'), {fontFamily: '" . $used_font . "'});";
			}
		}
	?>
	}
