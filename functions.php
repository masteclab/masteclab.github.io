<?php 
	...
	*/
	...
	// constants
	define('THEME_SHORT_NAME','sld'); 
	define('THEME_FULL_NAME','SlideOne');
	...
	$gdl_menu_string = get_option(THEME_SHORT_NAME.'_create_menu');
	if( !empty( $gdl_menu_string ) ){
		$gdl_menu_xml = new DOMDocument();
		$gdl_menu_xml->loadXML($gdl_menu_string);
		$gdl_menu_node = $gdl_menu_xml->documentElement;
	}
	...
	include_once('include/plugin/page-management.php');	 // organize page element
	...
	include_once('include/plugin/ajax-comment.php'); // ajax comment
